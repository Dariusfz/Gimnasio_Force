...
class RunsAdapter(private val runsList: ArrayList<Carreras>) :RecyclerView.Adapter<RunsAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RunsAdapter.MyViewHolder, position: Int) {
        ...
        var day = run.fecha?.subSequence(8, 10)
        var n_month = run.fecha?.subSequence(5, 7)
        var month: String ?= null
        var year = run.fecha?.subSequence(0, 4)
        ...
        holder.tvStartTime.text = run.inicioTiempo?.subSequence(0,5)
        holder.tvDurationRun.text = run.duracion
        holder.tvHeaderDuration.text = run.duracion!!.subSequence(0,5).toString()+"HH"
        ...
        if (run.intervalMode != null){
            ...
            details += "${run.tiempoCorriendo}/${run.tiempoCaminando})"
        ...
        holder.tvDelete.setOnClickListener{
            ...
            currentRun.distancia = run.distancia
            currentRun.avgSpeed = run.avgSpeed
            currentRun.maxSpeed = run.maxSpeed
            currentRun.duracion = run.duracion
            currentRun.activatedGPS = run.activatedGPS
            currentRun.fecha = run.fecha
            currentRun.inicioTiempo = run.inicioTiempo
            currentRun.usuario = run.usuario
            currentRun.deporte = run.deporte

            deleteRunAndLinkedData(id, currentRun.deporte!!, holder.lyDataRunHeader, currentRun)
